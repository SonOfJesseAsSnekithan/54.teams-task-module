...
namespace Microsoft.BotBuilderSamples.Bots
{
    public class TeamsTaskModuleBot : TeamsActivityHandler
    {
        ...
        private async Task<DialogTurnResult> ShowProfileDialogAsync(DialogContext dc, CancellationToken cancellationToken)
        {
            
            var turn = await dc.BeginDialogAsync(nameof(UserProfileDialog), null, cancellationToken);
            return turn;
        }
        
        protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            if (turnContext?.Activity?.Text == "get profile")
            {
                

            }
            else if(turnContext?.Activity?.Text == "launch task")
            {
